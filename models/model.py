...
class Model(nn.Module):
    ...
    def __init__(self, num_classes=None, drop_last_stride=False, joint_training=False, neighbor_mode=1, **kwargs):
        ...
        if self.joint_training:
            ...
            self.beta_dist = torch.distributions.beta.Beta(self.alpha, self.alpha)
    ...
    def generate_mix_data(self, inputs, **kwargs):
        half_bs = inputs.size(0) // 2
        source_imgs = inputs[:half_bs]
        target_imgs = inputs[half_bs:]

        self.mix_factor = self.rng.beta(self.alpha, self.alpha)
        mixed_imgs = self.mix_factor * source_imgs + (1 - self.mix_factor) * target_imgs
        return mixed_imgs
    ...
    def forward(self, batch, **kwargs):
        ...
        if kwargs.get('epoch') != self.epoch:
            self.rng = np.random.default_rng(kwargs.get('epoch'))
        ...
        inter_loss, inter_metric = self.mixed_st_forward(feats[:half_batch_size], label_s, img_ids[half_batch_size:], epoch)
        loss += inter_loss
        metric.update(inter_metric)
    ...
    def target_train_forward(self, feat, **kwargs):
        ...
        sim = MemoryBankSimCalc.apply(img_ids, feat, self.memory_linear.memory,
            self.mom if epoch > 5 else 0., dist.get_world_size())

        mask_instance = torch.zeros_like(sim)
        mask_instance[torch.arange(sim.shape[0]), img_ids] = 1
        mask_instance = mask_instance.bool()

        if not self.neighbor_mode:
            sim_neighbor = (sim.detach() + 1.) * (~ mask_instance) - 1.
            nearest = sim_neighbor.max(dim=1, keepdim=True)[0]
            mask_neighbor = sim_neighbor > (nearest * self.neighbor_eps)
            ...
            if self.nn_filter:
                confidence = sim.detach().mul(mask_neighbor).sum(dim=1).div(num_neighbor)
                confidence = confidence > self.thresh
                loss *= confidence

            loss -= score.masked_select(mask_instance)
            loss = loss.mean()
            if epoch <= 5: loss *= 0.
            return loss, {'mask_instance': mask_instance}
        else:
            mask_inter = torch.ones_like(sim)
            for i, cam in enumerate(cam_ids.tolist()):
                mask_inter[i, self.cam2uid[cam]] = 0
            mask_inter = mask_inter.bool()
            mask_intra = (~ mask_inter)

            sim_exp = torch.exp(sim * self.scale)

            # intra loss
            sim_intra = (sim.detach() + 1.) * mask_intra * (~ mask_instance) - 1.
            nearest_intra = sim_intra.max(dim=1, keepdim=True)[0]
            mask_neighbor_intra = sim_intra > (nearest_intra * self.neighbor_eps)
            ...
            score_intra.clamp_min_(1e-8)
            intra_loss = -score_intra.log().mul(mask_neighbor_intra).sum(dim=1).div(num_neighbor_intra)
            if self.nn_filter:
                confidence_intra = sim.detach().mul(mask_neighbor_intra).sum(dim=1).div(num_neighbor_intra)
                confidence_intra = confidence_intra > self.thresh
                intra_loss *= confidence_intra

            # instance loss
            instance_loss = -score_intra.masked_select(mask_instance).log()

            # inter loss
            sim_inter = (sim.detach() + 1.) * mask_inter - 1.
            nearest_inter = sim_inter.max(dim=1, keepdim=True)[0]
            mask_neighbor_inter = sim_inter > (nearest_inter * self.neighbor_eps)
            ...
            score_inter.clamp_min_(1e-8)
            inter_loss = -score_inter.log().mul(mask_neighbor_inter).sum(dim=1).div(num_neighbor_inter)
            if self.nn_filter:
                confidence_inter = sim.detach().mul(mask_neighbor_inter).sum(dim=1).div(num_neighbor_inter)
                confidence_inter = confidence_inter > self.thresh
                inter_loss *= confidence_inter

            loss = instance_loss + intra_loss * self.loss_factor[0] + inter_loss * self.loss_factor[1]
            loss = loss.mean()
            if epoch <= 5: loss = 0. * loss

            # mask = {
            #     'mask_instance': mask_instance,
            #     'mask_intra': mask_intra,
            #     'mask_neighbor_intra': mask_neighbor_intra,
            #     'mask_inter': mask_inter,
            #     'mask_neighbor_inter': mask_neighbor_inter
            # }
            self.mask_instance = mask_instance
            self.mask_intra = mask_intra
            self.mask_neighbor_intra = mask_neighbor_intra
            self.mask_inter = mask_inter
            self.mask_neighbor_inter = mask_neighbor_inter

            return loss, {
                'instance_loss': instance_loss.mean().item(),
                'intra_loss': intra_loss.mean().item(),
                'inter_loss': inter_loss.mean().item()
            }
    ...
    def mixed_st_forward(self, feat, labels, img_ids, epoch):
        cls_score = self.classifier(feat)

        if epoch == 1 or not self.mix_st:
            loss = F.cross_entropy(cls_score, labels)
            return loss, {'id_ce': loss.item()}
        else:
            if not self.neg_proto:
                if self.transition:
                    pos_agent = self.trans_dist(self.memory_linear.memory[img_ids], self.norm_mem.buffer[img_ids])
                else:
                    pos_agent = self.memory_linear.memory[img_ids]

                pos_agent = feat.mul(pos_agent).sum(dim=1, keepdim=True).mul(
                    self.classifier.weight[labels].detach().norm(dim=1, keepdim=True))
                cls_score = torch.cat([cls_score, pos_agent], dim=1)
            else:
                mask_non_neighbor = (~ self.mask_instance) * (~ self.mask_neighbor_intra) * \
                    (~ self.mask_neighbor_inter)
                num_neg_agent = min(self.classifier.weight.shape[0] - 1, mask_non_neighbor.sum(dim=1).min().item())
                agent = []
                for i in range(feat.shape[0]):
                    indices_selected = self.rng.choice(mask_non_neighbor[i].nonzero().squeeze().tolist(),
                                                       num_neg_agent, replace=False).tolist()
                    indices_selected.insert(0, img_ids[i].item())
                    if self.transition:
                        agent.append(self.trans_dist(self.memory_linear.memory[indices_selected], self.norm_mem.buffer[indices_selected]))
                    else:
                        agent.append(self.memory_linear.memory[indices_selected])

                agent = torch.stack(agent, dim=0)
                agent = torch.einsum('ik,ijk->ij', feat, agent).mul(
                    self.classifier.weight[labels].detach().norm(dim=1, keepdim=True))
                cls_score = torch.cat([cls_score, agent], dim=1)

            virtual_pids = labels.clone().fill_(self.classifier.weight.shape[0])
            loss = self.mix_factor * F.cross_entropy(cls_score, labels) + (1 - self.mix_factor) * \
                F.cross_entropy(cls_score, virtual_pids)
...
class MemoryBankSimCalc(torch.autograd.Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, indices, feats, memory_bank, momentum, world_size):
        if world_size > 1:
            indices_list = [torch.empty_like(indices) for _ in range(world_size)]
            feats_list = [torch.empty_like(feats) for _ in range(world_size)]

            dist.all_gather(indices_list, indices)
            dist.all_gather(feats_list, feats)

            all_indices = torch.cat(indices_list, dim=0)
            all_feats = torch.cat(feats_list, dim=0)

            ctx.save_for_backward(all_indices, all_feats, memory_bank)
        else:
            ctx.save_for_backward(indices, feats, memory_bank)

        ctx.momentum = momentum
        return torch.mm(feats, memory_bank.t())

    @staticmethod
    @custom_bwd
    def backward(ctx, grads):
        indices, feats, memory_bank = ctx.saved_tensors

        grad_out = None
        if ctx.needs_input_grad[1]:
            grad_out = torch.mm(grads, memory_bank)

        memory_bank[indices] *= ctx.momentum
        memory_bank[indices] += (1 - ctx.momentum) * feats
        memory_bank[indices] /= torch.norm(memory_bank[indices], p=2, dim=1, keepdim=True)
